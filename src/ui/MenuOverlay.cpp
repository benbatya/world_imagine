...
void MenuOverlay::draw(AppState& state) {
    ...
    if (ImGui::BeginPopupModal("Import Splats##dlg", &importOpen,
                               ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (ImGui::Button("Import", {120, 0})) {
            try {
                ...
                state.setStatus("Loaded " +
                    std::to_string(state.gaussianModel->numSplats()) + " splats");
    ...
    if (ImGui::BeginPopupModal("Export Splats##dlg", &exportOpen,
                               ImGuiWindowFlags_AlwaysAutoResize)) {
