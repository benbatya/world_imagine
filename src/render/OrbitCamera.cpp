...
glm::vec3 OrbitCamera::position() const {
  return target_ + orientation() * glm::vec3{0.f, 0.f, distance_};
}
...
glm::mat4 OrbitCamera::view() const {
  ...
  glm::vec3 eye   = target_ + back * distance_;
...
glm::mat4 OrbitCamera::proj(float aspect) const {
  float f = 1.f / std::tan(fovY_ * 0.5f);
  float n = zNear_, r = zFar_;
...
void OrbitCamera::moveKeyboard(float fwd, float rgt, float upv, float dt) {
  ...
  target_ += right * (rgt * scale) + glm::vec3{0.f, 1.f, 0.f} * (upv * scale);
}

// TODO: fitToBounds changes the default distance and target. 
// These should be preserved and used inside reset()
void OrbitCamera::reset() {
  target_      = {0.f, 0.f, 0.f};
  distance_    = 5.f;
  moveSpeed    = 2.f;
  orientation_ = glm::normalize(glm::angleAxis(0.f, glm::vec3{0.f, 1.f, 0.f}) *
                                glm::angleAxis(0.f, glm::vec3{1.f, 0.f, 0.f}));
}

void OrbitCamera::set(const glm::vec3& position, const glm::quat& orientation) {
  orientation_ = glm::normalize(orientation);
  target_      = position - orientation_ * glm::vec3{0.f, 0.f, distance_};
}
...
void OrbitCamera::fitToBounds(glm::vec3 center, float radius) {
  if (radius < 1e-4f)
    radius = 1.f;
  target_   = center;
  distance_ = radius * 2.5f;
  zNear_    = radius * 0.001f;
  zFar_     = radius * 20.f;
